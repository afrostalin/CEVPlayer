...
void CVideoPlugin::AddListener(IVideoPlayerEventListener * pVideoPlayerListener)
{
	if (mEnv != nullptr && mEnv->pVideoQueue != nullptr)
	{
		if (pVideoPlayerListener != nullptr)
...
void CVideoPlugin::RemoveListener(IVideoPlayerEventListener * pVideoPlayerListener)
{
	if (mEnv != nullptr && mEnv->pVideoQueue != nullptr)
	{
		if (pVideoPlayerListener != nullptr)
		{
			mEnv->pVideoQueue->Unregister2DVideoPlayerListener(pVideoPlayerListener);
		}
	}
}
